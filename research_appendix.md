...
A backend environment file was added at `backend/.env` with runtime keys required for database connectivity and LLM roadmap generation:
...
This supports a stable startup path for Flask services and ensures LLM-first roadmap generation can run with a deterministic fallback path when API output is invalid.

### Seeding and Demo Behavior Setup

The seeding workflow was extended to include:

- Demo roadmap data in `roadmaps`
- Demo progress data in `progress`
- Resource type normalization into supported values (`video`, `article`, `project`)

This allows immediate local validation of adaptation behavior without manual data creation.

### Adaptation Engine Strengthening

Adaptation logic now explicitly logs and applies:

- Content-type usage and skip counts
- Video skip threshold behavior (`video` skipped more than two times)
- Remedial-step insertion before failed steps when attempts are high

Outputs now include both an updated roadmap and a clear changes log for explainability and auditability.

### API Operability Hardening

Backend API paths were stabilized by:

- Moving route logic to service layer wrappers where required
- Improving user ID handling across string/ObjectId storage patterns
- Adding safer validation/handling for route parameters used in DB lookups
